...
def train_and_evaluate_gru(params, d):
    ...
    model = MultiTaskGRUNet(d['X_train'].shape[1], params['hidden_size'], params['num_layers'])
    ...
    for epoch in range(50):
        model.train()
        for batch_x, b_out, _, _, _ in train_loader:
            optimizer.zero_grad()
            p_out, _, _, _ = model(batch_x)
            l = loss_outcome(p_out, b_out)
            l.backward()
            optimizer.step()
        ...
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), os.path.join(MODELS_SAVED_DIR, 'temp_gru.pth'))
    return best_f1

def run_tuning_gru():
    d = get_prepared_data()
    param_grid = {'lr': [0.001, 0.0005], 'hidden_size': [64, 128], 'num_layers': [1, 2]}
    keys, values = zip(*param_grid.items())
    combinations = [dict(zip(keys, v)) for v in itertools.product(*values)]
    
    results = []
    print("Начинаем перебор комбинаций для GRU...")
    for i, params in enumerate(combinations):
        f1 = train_and_evaluate_gru(params, d)
        params['f1_macro'] = f1
        results.append(params)
        print(f"Тест {i+1}: {params} -> F1: {f1:.4f}")

    BASE_SAVE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../models/saved'))
    os.makedirs(BASE_SAVE_DIR, exist_ok=True)

    results_df = pd.DataFrame(results).sort_values(by='f1_macro', ascending=False)
    tuning_path = os.path.join(BASE_SAVE_DIR, 'tuning_gru_results.csv')
    results_df.to_csv(tuning_path, index=False)
    
    best_params = results_df.iloc[0].to_dict()
    print(f"\nЛУЧШИЕ ПАРАМЕТРЫ GRU: {best_params}")

    save_config(best_params, d, 'gru') 

    final_model_path = os.path.join(BASE_SAVE_DIR, 'best_gru_model.pth')
    temp_path = os.path.join(BASE_SAVE_DIR, 'temp_gru.pth')
